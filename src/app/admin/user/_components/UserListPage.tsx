...
import { _userDeleteAction } from "@/_actions/UserActions";
import { toast } from "react-toastify";
import LoaderPrimary from "@/_components/loaders/LoaderPrimary";
import PaginationPrimary from "@/_components/pagination/PaginationPrimary";
import { RolesData, UserRole } from "@/_data/sample/RolesData";
...
export default function UserListPage({dbData}: {dbData: any}) {
  ...
  return (
    ...
    <section className='mx-auto w-[92%]'>
      ...
      <SpacerTertiary />
          ...
          {/* Search Bar */}
          <section className="flex lg:flex-row flex-col items-user justify-between gap-4 mb-4">
          ...
          { dataList && dataList.length > 0  ? 
            <section className="w-full lg:overflow-hidden overflow-scroll">
              <div className='lg:w-[100%] w-[70rem]'>
                {/* HEADER */}
                <section className="w-full bg-gray-300 font-bold text-lg border border-gray-400 flex items-user justify-start">
                ...
                {/* ITEMS */}
                {dataList.map((i, key) => (
                  <section key={key} className="w-full border-x border-b border-gray-400 flex items-user justify-start">
