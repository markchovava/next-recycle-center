...
import { _faqDeleteAction } from "@/_actions/FaqActions";
...
export default function FaqListPage({ dbData }: { dbData: any }) {
            ...
            <div className='lg:w-[100%] w-[70rem]'>
              ...
              {dataList.map((i, key) => (
                <section key={key} className="w-full border-x border-b border-gray-400 flex items-center justify-start">
                  <div className="flex-5 border-r border-gray-400 px-2 py-1">{trimString(i.question, 24)}</div>
                  <div className="flex-3 border-r border-gray-400 px-2 py-1">
                    {i.user.name ?? i.user.email}
                  </div>
                  <div className="flex-4 border-r border-gray-400 px-2 py-1">{formatDate(i.createdAt)}</div>
